...
const fetchClaims = async () => {
  ...
  setLoading(true);
  try {
    const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/employee/claims`, {
 ...
 useEffect(() => {
  ...
  const handleClaimSubmit = async (e) => {
    ...
    try {
      ...
      let url = `${import.meta.env.VITE_API_BASE_URL}/employee/claims`;
      if (newClaim.id) {
        url = `${import.meta.env.VITE_API_BASE_URL}/employee/claims/update`;
                      ...
                      {newClaim.existingDocuments.map((doc, index) => (
                        <div key={index} className="col-md-6">
                          <div className="d-flex justify-content-between align-items-center p-2 border rounded bg-light">
                            ...
                            <div>
                              <a href={`${import.meta.env.VITE_API_BASE_URL}${doc}`} target="_blank" rel="noopener noreferrer"
