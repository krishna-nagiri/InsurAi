...
const fetchClaims = async () => {
  ...
  setLoading(true);
  try {
    const res = await fetch("http://localhost:8090/employee/claims", {
 ...
 useEffect(() => {
  ...
  const handleClaimSubmit = async (e) => {
    ...
    try {
      ...
      let url = "http://localhost:8090/employee/claims";
      if (newClaim.id) {
        url = "http://localhost:8090/employee/claims/update";
                      ...
                      {newClaim.existingDocuments.map((doc, index) => (
                        <div key={index} className="col-md-6">
                          <div className="d-flex justify-content-between align-items-center p-2 border rounded bg-light">
                            ...
                            <div>
                              <a href={`http://localhost:8090${doc}`} target="_blank" rel="noopener noreferrer"
