...
const HRFraud = () => {
  ...
  const fetchFraudAlerts = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/hr/claims/fraud`, {
        headers: {
                    ...
                    <tr key={a.id} onClick={() => setViewingAlert(a)} className="cursor-pointer">
                      ...
                      <td>
                        {a.documents?.map((doc, i) => (
                          <a key={i} href={`${import.meta.env.VITE_API_BASE_URL}${doc}`} target="_blank" className="d-block">
                            Doc {i + 1}
                          </a>
                        ))}
                      </td>
              ...
              <div className="modal-body">
                ...
                <ul>
                  {viewingAlert.documents?.map((doc, i) => (
                    <li key={i}><a href={`${import.meta.env.VITE_API_BASE_URL}${doc}`} target="_blank">Document {i + 1}</a></li>
                  ))}
                </ul>
              </div>
