...
export default function HRDashboard() {
  ...
  const fetchEmployees = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/auth/employees`);
  ...
  const fetchHRList = useCallback(async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/hr`);
  ...
  const fetchPolicies = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/employee/policies`, {
  ...
  const fetchClaims = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/hr/claims?hrId=${loggedInHrId}`, {
  ...
  const approveClaim = async (id, remarks) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/hr/claims/approve/${id}`, {
        method: "POST",
        headers: { 
  ...
  const rejectClaim = async (id, remarks) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/hr/claims/reject/${id}`, {
        method: "POST",
        headers: { 
          "Authorization": `Bearer ${token}`,
          "Content-Type": "application/json"
  ...
  const downloadCSV = () => {
    ...
    const rows = displayedClaims.map(c => [
      ...
      c.priority,
      c.daysPending,
      c.documents?.length > 0 ? c.documents.map(d => `${import.meta.env.VITE_API_BASE_URL}${d}`).join(" | ") : "No documents"
    ]);
